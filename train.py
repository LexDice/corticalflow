...
def train_app(cfg):
    ...
    train_dataset = csr_dataset_factory('formatted', cfg.dataset.surface_name.split('.')[0], transforms, dataset_path=cfg.dataset.path, split_file=cfg.dataset.split_file, split_name=cfg.dataset.train_split_name, surface_name=cfg.dataset.surface_name)
    ...
    val_dataset = csr_dataset_factory('formatted', cfg.dataset.surface_name.split('.')[0], transforms, dataset_path=cfg.dataset.path, split_file=cfg.dataset.split_file, split_name=cfg.dataset.val_split_name, surface_name=cfg.dataset.surface_name)
    ...
    for deform_train_idx  in range(last_train_deform_idx if cfg.trainer.resume else 0, num_of_deformations):        
        ...
        for param in model.parameters():
            ...
            model_num_params += param.numel()

        for param in model.deform_blocks[deform_train_idx].parameters():
            param.requires_grad=True
            model_num_lr_params += param.numel()
