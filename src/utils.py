...
def import_mesh(filename_with_ext):
    ...
    assert mesh_ext in ['stl', 'obj', 'npz', 'white', 'pial', 'vtk', 'vtp'], "{} format is not supported".format(mesh_ext)
    ...
    if mesh_ext in ['stl', 'obj']:
        ...
        vertices, faces = mesh['vertices'], mesh['faces']
    elif mesh_ext in ['vtk', 'vtp']:
        mesh = pv.read(filename_with_ext)
...
def export_mesh(vertices, faces, filename_no_ext, mesh_fmt, white_or_pial=None):    
    ...
    if mesh_fmt == 'stl':
        out_mesh = tri.Trimesh(vertices, faces, process=False)
